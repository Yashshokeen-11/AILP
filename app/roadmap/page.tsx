...
const Teacher3D = dynamic(
  () =>
    import("@/components/teacher-3d").then((mod) => ({
      default: mod.Teacher3D,
    })),
  {
    ssr: false,
    loading: () => (
...
export default function RoadmapPage() {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      {/* Main Content Area - Takes 2/3 of screen */}
      ...
      {/* Right Sidebar - 3D Teacher (1/3 of screen) */}
      <aside className="hidden lg:flex w-1/3 border-l border-pink-300/30 dark:border-pink-700/30 bg-gradient-to-br from-card via-pink-50/50 dark:via-pink-950/20 to-pink-100/30 dark:to-pink-950/10 flex-col items-center justify-center p-8 sticky top-0 h-screen">
        <div className="w-full h-full max-w-lg flex flex-col">
          <div className="mb-6 text-center">
            <h3 className="text-3xl font-bold text-foreground mb-2 bg-gradient-to-r from-pink-500 to-purple-500 bg-clip-text text-transparent">
              Your AI Teacher
            </h3>
            <p className="text-sm text-muted-foreground">
          ...
          <div className="flex-1 flex items-center justify-center min-h-[500px]">
            <Teacher3D className="rounded-xl border-2 border-pink-300/40 dark:border-pink-700/40 bg-card/90 backdrop-blur-sm shadow-2xl" />
          </div>
        </div>
      </aside>
    </div>
